...
- 상품 상세: 사진 캐러셀, 원산지/중량/보관법/조리법 표, 설명, **상세설명(긴 스크롤형 이미지+텍스트, 더미 데이터)**, 수량 선택 + 장바구니 담기
...
- **상품 상세설명**: UI와 데이터 구조(`ProductDetailBlock`)는 완성됐지만 현재는 더미 콘텐츠만 노출됨. 관리자가 실제로 작성할 수 있는 기능은 아직 없음
