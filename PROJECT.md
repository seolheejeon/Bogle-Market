...
| `event_products` | id, event_id, product_id(→products), price, delivery_type, visible, closed, sort_order | **이벤트별 상품 리스팅** — 옛 `products` 테이블이 하던 역할(이벤트에 속한 판매 정보)을 그대로 가져오되 내용은 `product_id`로 카탈로그를 참조. `price`/`delivery_type`/`visible`/`closed`는 리스팅마다 독립적이라, 같은 카탈로그 상품도 회차별로 다른 가격·노출 여부·마감 여부를 가질 수 있음(재고는 없음 — `products.stock` 참고, Epic 1 Phase 3 전엔 여기 있었음). `delivery_type`이 비어있으면 소속 이벤트의 배송방식을 그대로 따름. `visible=false`면 고객 화면에서 이 리스팅만 숨김(카탈로그 상품 자체나 다른 회차 리스팅엔 영향 없음) — RLS도 `visible or is_admin()`으로 비노출 리스팅을 비관리자에게 숨김. `closed=true`는 `visible`과 반대로 화면엔 계속 보이되 주문만 막는 게이트(예약상품 발주마감 등, 재고와 무관) — 이벤트 전체의 `events.status`와는 별개로 리스팅 하나만 마감시킬 때 씀, `create_order` RPC가 여기 걸리면 트랜잭션 전체를 거부함. `sort_order`는 이 이벤트 안에서의 노출 순서(관리자 ▲▼, Epic 1 Phase 4) — 이벤트마다 독립적이고, 마감시키면 자동으로 맨 뒤로 재정렬됨. 화면(`Product` 타입)에는 `products`와 조인한 평평한 모양으로 합쳐져서 전달됨(`catalogProductId`로 원본 카탈로그 id를 알 수 있음) |
