...
- [ ] 상품 카드 배송예정 문구 개선
- [ ] 버전정보 / 앱정보 화면
- [ ] 상품 상세에서 여러 옵션 조합을 한 번에 선택하여 장바구니에 담기 지원
...
- [ ] 장바구니 데이터 정합성 개선
- [ ] 옵션 UX 개선
- [x] ~~홈/카테고리에서 빠른 담기 시 담기 완료 피드백 추가 (토스트 또는 장바구니 애니메이션)~~ → 완료(위 "완료된 기능" 참고)
- [x] ~~이벤트 관리 자동저장을 저장/취소 방식으로 변경~~ → 완료(위 "완료된 기능" 참고)
- [x] ~~저장되지 않은 변경사항 안내 및 페이지 이탈 방지~~ → 완료(`lib/useUnsavedChangesGuard.ts`, 이벤트 관리 화면에 적용)
- [x] ~~홈의 마감임박 상품은 마감까지 48시간 이하인 상품만 노출~~ → 완료(24시간 기준으로 구현 — 대화에서 24시간으로 확정)
- [x] ~~마감된 상품은 홈의 마감임박/인기상품에서 제외~~ → 완료
- [x] ~~마감임박 상품이 없으면 섹션 자체를 숨김~~ → 완료
...
- [x] 필수 옵션은 모두 선택해야 수량 변경 가능
- [x] 옵션 2개 이상은 옵션 조합별 재고 관리
- [x] 옵션 변경 시 수량 초기화 또는 옵션별 독립 수량 관리

## 장바구니

- [ ] 삭제/비노출 상품 제외 후 상품 개수 계산
- [ ] 장바구니 뱃지는 실제 상품 개수 기준
- [ ] 판매 종료 상품 자동 정리
...
마지막 업데이트: 2026-07-31 (상품 상세 다중 옵션조합 담기 UX — 여러 옵션 조합을 각각 골라 목록에 쌓은 뒤 한 번에 장바구니에 담는 의류 쇼핑몰식 UX로 변경)
...
**배송방식별 결제수단 제한**
- 배경: Epic 1(이벤트 관리/결제/재고 구조 개선)의 2번째 항목. 사다드림(무통장 계좌이체로만 운영)에도 카드/카카오페이/이음카드 결제가 그대로 노출되고 있어, 실제로는 받을 수 없는 결제수단을 고객이 선택할 수 있는 상태였음
- **`lib/payments.ts`에 `allowedPaymentMethods(deliveryType)` 추가** — 사다드림(`GROUP_BUY`)은 무통장입금 하나만, 문고리(`DOOR`)/택배(`PARCEL`)는 기존 4종(무통장/카드/카카오페이/이음카드) 모두 허용. 스키마 변경 없음(결제수단은 이미 주문마다 독립적으로 저장되던 필드라, 체크아웃 화면에서 올바른 값을 고르게만 하면 됨)
- **체크아웃 화면 재설계**(`CheckoutView.tsx`): 기존엔 결제수단을 장바구니 전체에서 딱 하나만 골랐는데, 이제 상품의 실질 배송유형(`product.deliveryType ?? event.type`)별로 묶어서 유형마다 따로 결제수단을 고르게 함 — 사다드림 섹션엔 무통장입금 버튼만 보이고, 문고리/택배 섹션엔 4종 모두 보임. 장바구니에 서로 다른 배송유형이 섞여 있으면 "결제 방식이 다른 상품이 포함되어 있습니다. 주문이 배송 유형별로 각각 생성됩니다." 안내 배너 표시
- **주문 생성**: 이벤트별로 별도 주문을 만드는 기존 구조(마감일/배송일이 달라 배송완료 처리를 독립적으로 하기 위함)는 그대로 유지 — 서로 다른 배송유형은 통상 서로 다른 이벤트에 속하므로 이벤트별 분리가 배송유형별 분리도 함께 만족시킴. 각 이벤트-주문에는 그 이벤트의 실질 배송유형에 맞게 선택된 결제수단이 적용됨
- 운영 환경에서 실제 이벤트 3종(문고리/사다드림/택배)을 한 장바구니에 담아 검증: 사다드림 섹션엔 무통장입금 버튼만 뜨는 것, 혼합 안내 배너가 뜨는 것, 문고리 섹션에서만 "카드 결제"로 바꿔도 사다드림/택배 선택엔 영향 없는 것, 무통장입금 선택 시에만 그 섹션 아래 계좌 정보가 뜨는 것 확인(주문 제출은 실행하지 않음 — 실 데이터 생성 방지)

**상품 중심 재고 구조**
- 배경: Epic 1의 3번째 항목. 예전엔 같은 상품을 여러 이벤트(예: 제육볶음을 8/1·8/2·8/3 문고리 세 회차)에 걸 때마다 재고를 매번 따로 입력해야 했고, 한 회차에서 주문이 들어가도 다른 회차 재고는 그대로 남아있어 실제 남은 물량과 화면상 재고가 서로 어긋날 수 있었음
- **재고를 `event_products`(리스팅)에서 `products`(카탈로그)로 이동** — 이제 이 상품을 파는 모든 이벤트가 재고 하나를 실시간으로 공유한다. 한 이벤트에서 주문이 들어가면 그 즉시(다음 조회부터) 같은 상품을 쓰는 다른 회차의 재고도 함께 줄어듦. `event_products`엔 이제 가격/노출/배송방식만 남음
- **재고 차감/복구 RPC(`decrement_stock`/`increment_stock`)는 그대로 리스팅 id(`event_product_id`)를 받되, 내부에서 그 리스팅이 가리키는 카탈로그 상품을 찾아 `products.stock`을 갱신**하도록 재작성 — 호출부(주문 생성/취소/환불 로직)는 코드 변경 없이 그대로 동작. `cancel_guest_order`의 재고 복구 로직도 동일하게 `event_products`를 거쳐 `products`를 갱신하도록 수정
- **재고 입력/수정은 "상품 관리"(`/admin/products`)에서만** — 이벤트 관리 화면(`/admin/events/[id]`)의 리스팅 편집에서는 재고 입력란과 "품절 처리" 버튼을 완전히 제거하고, 재고 숫자만 읽기 전용으로 보여주며 상품 관리로 안내하는 링크를 넣음. 이벤트에 새 리스팅을 추가할 때도 재고를 물어보지 않음(카탈로그 상품의 재고를 자동으로 공유)
- **상품 관리 화면에 "연결된 이벤트" 패널 추가** — 상품 목록에서 펼치면 이 상품이 걸려있는 이벤트 목록(배송유형·배송일·가격)을 보여주고, 회차별로 "연결 해제"(리스팅 제거) 가능. 드롭다운으로 아직 안 걸린 이벤트를 골라 가격만 입력하면 바로 그 회차에 추가할 수 있음
- **기존 리스팅별 재고값 이관**: 마이그레이션에서 상품별로 기존 `event_products.stock` 중 **가장 큰 값**을 새 `products.stock`으로 채움(여러 회차에 걸려 있던 상품이라면 그중 재고가 가장 넉넉했던 회차 기준)
...
- `tsc`/`build` 통과 확인. 마이그레이션 SQL 실행 및 배포 완료, 운영 환경에서 스키마 반영 확인함

**상품 상세 다중 옵션조합 담기 UX**
- 배경: 옵션 조합별 재고 구조가 자리잡은 뒤, 색상+사이즈처럼 옵션이 여러 개인 상품에서 "조합 하나 고르고 담기" → "또 다른 조합 고르고 담기"를 매번 페이지 이동 없이 반복할 수 있는 일반적인 의류 쇼핑몰 UX 요청
- **상품 상세에 "대기 목록"(pendingLines) 개념 추가** — 옵션이 있는 상품은 예전처럼 조합을 고르자마자 바로 장바구니에 담는 대신, "+ 이 옵션 담기"를 누르면 그 조합이 화면 안의 목록에 한 줄(조합명 + 수량 −/+ + 삭제)로 쌓이고 선택 상태는 비워져서 바로 다음 조합을 고를 수 있음. 이미 목록에 있는 것과 완전히 같은 조합을 또 담으면 새 줄을 만들지 않고 기존 줄의 수량만 1 늘림
- 목록 하단에 "총 N개 담기"가 있는 하단 고정 바가 있고, 눌러야만 목록에 쌓인 조합들이 한 번에 실제 장바구니에 반영됨(줄마다 `changeQty` 한 번씩 호출) — 그 전까지는 새로고침하면 목록이 사라지는 임시 상태
- 각 줄의 +버튼은 그 조합의 실제 남은 재고(이미 장바구니에 있는 수량 + 목록의 다른 줄에서 같은 재고를 쓰는 수량까지 함께 계산)를 넘지 않도록 막음 — 재고관리 대상이 아닌 값만 다른 두 조합이어도 재고 키가 같으면(예: 사이즈만 재고관리 중이고 색상은 무제한이면) 서로 같은 재고를 나눠 쓰는 것으로 정확히 계산됨
- 옵션이 없는 단순 상품은 예전처럼 수량 스텝퍼 + 바로 담기 그대로 유지(회귀 없음)
- `tsc`/`build` 통과 확인. 운영 환경 브라우저 검증은 아직 진행 전(배포 후 진행 예정)

**최소 구매 수량 + 상품별 택배 배송비**
- 배경: "땅콩빵 2개부터만 팔고 싶다" 같은 최소구매수량 제약과, 택배 상품이 감자/홍게/고구마처럼 판매자·산지가 제각각이라 상품마다 배송비·무료배송 기준·택배사·출고방식이 다 다른 실제 운영 상황을 반영해 달라는 요청
- **최소 구매 수량**: `products.min_qty`(기본 1) 추가. 상품 상세 진입/옵션 재선택/빠른 담기(그리드·이벤트 상세) 모두 `min_qty`로 시작하고, `QtyControl`의 "−"는 `minQty`에 도달하면 비활성화(0으로는 못 내려감 — 완전히 빼려면 장바구니의 별도 "삭제" 버튼 사용). 다중 옵션조합 대기목록(`pendingLines`)의 "−"도 동일하게 `minQty` 밑으로는 못 내려가게 막고, 새 조합을 대기목록에 처음 담을 때도 `minQty`개로 시작. 서버(`create_order` RPC)에서도 주문 항목의 수량이 상품별 `min_qty` 미만이면 주문 자체를 거부 — 클라이언트 우회와 무관하게 항상 강제됨
- **상품별 택배 배송비**: `products`에 `shipping_fee`/`free_shipping_threshold`/`courier_code`/`fulfillment_type`(당일발송·순차출고·출고예정일지정)/`ships_at` 추가 — 이벤트(회차)가 아니라 **카탈로그 상품 단위**로 정책을 관리(같은 택배 이벤트에 담겨도 상품마다 배송사·배송비가 다를 수 있어서). 계산 로직은 `lib/shipping.ts`(카탈로그 상품 id로 묶어서 상품당 배송비 한 번만 부과, 그 상품의 소계가 무료배송 기준을 넘으면 0원)로 단일화해 장바구니(`CartView`)·체크아웃(`CheckoutView`)이 항상 같은 방식으로 계산·표시함. 장바구니/체크아웃 모두 이벤트(주문)별로 소계·배송비·합계를 따로 보여주고, 체크아웃은 실제 결제될 금액과 화면에 보이는 총액이 항상 일치하도록 배송비를 포함해 계산. 상품 상세에는 택배 배송 시에만 "배송 안내"(택배사/출고방식/배송비/무료배송 기준) 박스 표시
- 주문 생성 시 `orders.shipping_fee`에 계산된 배송비를 스냅샷으로 저장(이후 상품 배송정책이 바뀌어도 과거 주문 금액은 그대로 유지) — `create_order` RPC에 `p_shipping_fee` 파라미터 추가
- 관리자 상품 등록/수정 화면(`/admin/products`)에 최소구매수량 입력란과, 택배로 팔 때만 적용되는 배송비/무료배송 기준금액/택배사(select)/출고방식(라디오, 출고예정일 지정 시 날짜 입력) 섹션 추가
- `tsc`/`build` 통과 확인. 마이그레이션 SQL 실행 완료(운영 환경 반영됨)
- 상품 상세 정보 표(원산지/중량/보관법/조리법)에 "최소주문수량" 행 추가 — `min_qty > 1`일 때만 "N개부터 주문 가능"으로 표시해서, 수량이 1이 아니라 N부터 시작하는 이유를 고객이 바로 알 수 있게 함(예: 중량 200g / 최소주문수량 2개부터 주문 가능 / 보관법 냉동보관 / 조리법 에어프라이어)
- **배송비 정책 3종으로 확장**: `products.shipping_fee_type`(기본 `fixed`)으로 상품마다 다른 배송비 계산 방식을 고를 수 있게 함 — ① `fixed`(고정 배송비, 항상 동일 금액) ② `free_threshold`(기존 방식 — 소계가 기준금액 이상이면 무료) ③ `per_quantity`(신규 — `shipping_fee_qty_unit`개마다 배송비 1회씩 추가 부과, 예: 5세트마다 4,000원이면 12개 주문 시 ceil(12/5)×4,000원). `lib/shipping.ts`의 `shippingFeeForProduct`가 이 세 방식을 모두 처리하는 단일 진입점이라 장바구니/체크아웃/상품상세가 항상 같은 계산 결과를 봄
- **택배사 직접 입력**: 관리자 상품 폼의 택배사 select에 "직접 입력" 옵션을 추가 — 선택하면 텍스트 인풋이 나타나고, 입력한 값이 그대로 `courier_code`에 저장됨(기본 목록: CJ대한통운/한진택배/롯데택배/우체국택배/로젠택배). 이 필드는 상품 상세의 "배송 안내" 표시용일 뿐 실제 송장 조회(주문의 `courier_code`/스마트택배 API)와는 별개라 자유 텍스트를 허용해도 안전함 — 고객 화면은 이미 `COURIER_LABEL[code] ?? code`로 폴백하고 있어서 별도 수정 없이 자유 텍스트도 그대로 표시됨
...
**관리자 이벤트 수정: 자동저장 → 저장/취소**
- 배경: 이벤트 관리 화면(`/admin/events/[id]`)의 이름/뱃지/마감/배송일/안내문구가 입력 후 포커스를 벗어나는 즉시 저장돼서, 운영해보니 실수로 값을 바꾸자마자 반영되는 게 오히려 불편하다는 피드백. 여러 항목을 고친 뒤 한 번에 저장하는 일반적인 관리자 폼 UX로 바꿔달라는 요청
- **로컬 draft 상태로 전환**: 상단 정보 블록만 `EventDraft`(이름/뱃지/마감/배송일/안내문구)로 편집 — 입력은 전부 로컬 상태만 바꾸고, "저장" 버튼을 눌러야만 `updateEvent`가 호출됨. `draft`는 화면 최초 진입 시 한 번만 채워지고, 이후 상품 추가/제거/순서변경/가격수정 등 **다른 조작이 페이지를 다시 불러와도(`refresh()`) draft를 덮어쓰지 않음** — 그래야 "이름 고치는 중에 다른 상품 노출을 껐다 켰다" 해도 이름 수정분이 날아가지 않음
- **변경사항 감지**: 현재 draft와 저장된 이벤트 값을 비교(`draftsEqual`)해서 다른 점이 있을 때만 저장 버튼이 활성화되고, 없으면 버튼이 비활성화된 채 "변경사항 없음"이 표시됨
- **취소**: 마지막으로 불러온 저장 상태(`event`)로 draft를 되돌림 — 서버에는 아무 요청도 안 감
- **저장 성공 토스트**: "✓ 저장되었습니다."를 헤더 옆에 잠깐 보여줌(기존의 "저장 중/완료/실패" 3단 표시를 대체)
- **페이지 이탈 방지**(`lib/useUnsavedChangesGuard.ts`, 재사용 가능한 훅으로 분리): 변경사항이 있는 상태에서 새로고침/탭 닫기는 `beforeunload`로 브라우저 기본 확인창을, 앱 안에서 다른 페이지로 가는 `<Link>` 클릭은 문서 레벨에서 클릭을 캡처 단계에 가로채 직접 `confirm()`을 띄우고 취소 시 이동을 막는 방식으로 처리(클라이언트 라우팅은 `beforeunload`가 안 뜨기 때문에 이 부분이 따로 필요함)
- **범위**: 상품별 "가격/원가 수정"(이미 자체 저장/취소 버튼이 있던 방식)과 옵션 조합별 재고 입력(의도적으로 blur 즉시저장 — 실사 중 빠르게 여러 개를 입력하는 용도)은 건드리지 않음, 요청받은 자동저장 문제는 상단 정보 블록에 한정된 증상이라 그대로 둠
...
**마이페이지: 프로필 정보 수정 + 배송지 통합**
- 배경: "배송지도 회원정보의 일부"라는 판단 아래, 예전엔 "정보 수정"(닉네임/휴대폰/비밀번호)과 "기본 배송지"(도로명주소/상세주소/공동현관/배송메모)가 항상 펼쳐진 별도 카드 두 개로 나뉘어 있던 걸 하나로 합쳐달라는 요청. 데이터 구조(프로필/주소가 별도 테이블인 것)는 그대로 두고 화면 구성만 바꿈
- **요약 화면은 그대로 간단히**: 로그인 후 첫 화면에는 닉네임/아이디/휴대폰번호만 보여주고 배송지 카드는 아예 안 보임(예전엔 배송지 입력폼이 항상 펼쳐져 있었음)
- **"정보 수정" 클릭 시 한 화면에서 전부 편집**: 기본 정보(닉네임/휴대폰) → 배송지(주소검색/상세주소/공동현관 출입방법/배송메모) → 비밀번호 변경, 이렇게 섹션 제목으로 구분된 하나의 폼 안에 전부 들어있고 "저장" 버튼 하나로 `updateProfile` → (입력했으면) `changePassword` → `updateAddress`를 순서대로 한 번에 처리함. "취소"를 누르면 프로필/배송지 입력값이 전부 마지막 저장 상태로 되돌아감(주소는 `savedAddress`에서 다시 채움)
- **확장 구조**: 각 섹션이 "제목 + 입력란들"의 같은 모양 블록이라, 나중에 "알림 설정"이나 다른 개인정보 항목을 추가하고 싶으면 비밀번호 섹션 앞뒤에 같은 모양 블록 하나만 더 넣으면 됨(코드에 이 의도를 주석으로 남겨둠)
- `tsc`/`build` 통과 확인. 로그인 전 화면(회원가입/로그인 폼)은 이 변경과 무관해서 브라우저로 정상 렌더링/콘솔 에러 없음까지 확인함 — 로그인 후 프로필 화면(정보 수정 펼치기/저장/취소)은 계정 정보가 없어 직접 클릭 테스트는 못 하고 코드 검토로 대신함
- 마이그레이션 불필요(DB 스키마 변경 없음, 기존 profiles/addresses 테이블 그대로 사용)

**홈 화면 UX 개선 (마감임박/인기상품 필터링 + 장바구니 피드백/카운트)**
- 배경: 실제 운영해보니 홈 화면의 "마감 임박 상품"/"인기상품"에 이미 마감된 이벤트가 계속 떠 있고, 홈/카테고리의 빠른 담기(+ 버튼)는 눌러도 아무 피드백이 없어 담겼는지 알기 어렵고, 장바구니 아이콘 숫자가 삭제/비활성 상품 때문에 실제와 안 맞는 경우가 있다는 네 가지 피드백
- **마감 임박 상품 필터링**(`HomeView.tsx`): 예전엔 문고리/사다드림/특가 각각 "가장 가까운 이벤트" 딱 하나씩(최대 3개)을 무조건 보여줬는데, 마감 여부·임박 정도와 무관했다. 이제는 전체 이벤트를 대상으로 `isEventOrderable`(마감 안 지남) + 마감까지 24시간 이내인 것만 걸러서 마감이 가까운 순으로 보여줌 — 문고리처럼 마감 지나도 재고 있으면 계속 팔리는 이벤트(`SOFT_DEADLINE`)도 "임박" 표시에서는 실제 마감 시각 기준으로 정확히 판단함. 택배(PARCEL)는 상시판매라 마감 개념이 없어 애초에 대상에서 제외. 조건에 맞는 이벤트가 하나도 없으면 섹션 자체가 안 보임(기존에 이미 `length > 0` 조건부 렌더링이라 필터만 고치면 자동으로 해결됨)
- **인기상품 필터링**: 마감된(주문 불가) 이벤트의 상품을 아예 제외 — 예전엔 마감 상품도 목록에 넣고 "마감" 뱃지만 얹어서 보여줬음
- **장바구니 담기 피드백**(`lib/cart-feedback.ts` 신설): 홈/카테고리/이벤트 상세의 빠른 담기(`QtyControl`)에 상품 상세 페이지에 이미 있던 것과 같은 "장바구니에 담았습니다." 토스트 + 상품 사진이 헤더 장바구니 아이콘으로 날아가는 애니메이션을 적용함. 그리드에는 `QtyControl`이 여러 개 동시에 떠 있을 수 있어서, 토스트는 컴포넌트별 React state가 아니라 문서에 하나만 만들어 재사용하는 방식으로(화면에 토스트가 여러 개 동시에 뜨는 것을 방지) 구현. 상품 상세 페이지가 원래 쓰던 fly-to-cart 함수를 그대로 이 공용 모듈로 옮겨서 중복을 없앰(동작은 동일)
- **장바구니 카운트 정합성**(`lib/cart-context.tsx`): 삭제되거나 노출을 꺼둔 상품이 장바구니에 남아있으면 헤더 배지/합계가 실제로 살 수 없는 수량까지 세던 문제 — 하이드레이션 직후 한 번 `listEvents()`로 지금 존재하고 노출 중인 상품 목록과 대조해서, 더 이상 유효하지 않은 장바구니 줄을 조용히 지움(헤더 배지뿐 아니라 장바구니 화면 합계 등 `count`/`lines`를 쓰는 모든 곳에 자동으로 반영됨). 네트워크 오류로 목록을 못 가져오면 기존 장바구니를 그대로 두고 조용히 넘어감(장바구니를 함부로 비우지 않기 위함)
- `tsc`/`build` 통과 확인. 프리뷰에서 실제 홈 화면을 열어 마감된 이벤트 2개가 인기상품/마감임박에서 정확히 빠지는 것, 인기상품 빠른 담기(+) 클릭 시 장바구니 배지가 0→1로 바뀌고 "장바구니에 담았습니다." 토스트가 뜨는 것, 콘솔 에러 없는 것까지 브라우저로 직접 확인함
- 마이그레이션 불필요(DB 스키마 변경 없음)

**푸시 알림 "받기" 버튼 무반응 버그 수정 + 상품 관리 화면 섹션 정리**
- 배경: 알림 화면에서 "받기" 버튼을 눌러도 반응이 없어 비활성화된 것처럼 보인다는 신고, 그리고 상품 관리 화면에서 문고리/사다드림/택배 이모지를 없애고 정보를 섹션별로 나눠달라는 요청
- **푸시 버튼 진단 결과**: 실제 배포 사이트를 직접 열어 확인해보니 원인은 `Notification.permission`이 이미 `"denied"`(브라우저에서 알림을 이전에 거부한 상태)였고, 예전 코드가 `disabled={busy || permission === "denied"}`로 버튼 자체를 눌러도 아무 일도 안 일어나게 막아놓은 것이었음. 서비스워커 등록(`swRegistered: true`, `active` 상태)·`PushManager` 지원 여부·VAPID 키 로드 상태는 모두 정상이었고, 콘솔 에러도 이전/이후 모두 전혀 없었음(원래 콘솔에 에러를 던지던 버그가 아니라, "조용히 아무것도 안 하는" 설계상의 문제였음)
- **수정 (`lib/push.ts`)**: `enablePush`가 실패 시 항상 구체적인 이유(`PushEnableFailureReason`: insecure/unsupported/no_vapid_key/permission_denied/sw_registration_failed/subscribe_failed)와 사람이 읽을 메시지(`PUSH_FAILURE_MESSAGE`)를 반환하도록 재작성(예전엔 실패하면 그냥 `null`). `navigator.serviceWorker.ready`는 서비스워커 등록이 한 번도 성공한 적 없으면 영원히 안 풀리는 프라미스라 이 지점에서 화면이 멈춘 것처럼 보일 수 있어서, 등록이 없으면 직접 `register()`를 시도하고 일정 시간(8초) 안에 활성화가 안 되면 타임아웃으로 확실히 실패 처리하는 `getReadyRegistration()` 추가
- **수정 (`app/(shop)/notifications/page.tsx`)**: `PushOptIn`의 버튼은 이제 이 브라우저가 애초에 Push API를 지원 안 하거나(HTTPS 아님/API 없음) 요청 처리 중(`busy`)일 때만 비활성화되고, `permission === "denied"`여도 항상 클릭 가능 — 눌러서 `Notification.requestPermission()`이 다시 뜨는 것 자체가 사용자에게 유용하기 때문. 실패하면 화면에 빨간 글씨로 구체적인 이유를 바로 보여줌
- **상품 관리 화면 정리 (`app/admin/products/page.tsx`)**: `CatalogProductForm`을 기본 정보 / 판매 정보 / 택배 정보 세 섹션으로 나눠 가독성 개선. 이모지 제거 요청은 확인해보니 상품 관리 화면(및 코드 전체)에 애초에 배송방식 이모지가 없었음(문고리/사다드림/택배는 어디서나 텍스트 라벨로만 표시됨) — 실제로 지울 것은 없었고 섹션 구분만 적용함
- `tsc`/`build` 통과 확인. 푸시 버튼은 실 배포 사이트 및 로컬 프로덕션 빌드에서 직접 클릭해 에러 메시지 노출까지 확인. 상품 관리 화면은 관리자 로그인이 필요해 이번 세션에서는 브라우저로 직접 확인하지 못함(기존과 동일한 한계)
- 마이그레이션 불필요(DB 스키마 변경 없음)

**모바일 이벤트 상품 목록 레이아웃 + 택배 이벤트 단순화 + 뱃지를 이벤트에서 상품으로 이동**
- 배경: 관리자 이벤트 수정 화면의 상품 목록이 PC 레이아웃을 그대로 모바일에 축소해 쓰다 보니 상품명이 세로로 줄바꿈되고 버튼이 좁아져 가독성이 떨어진다는 신고, 택배는 상품(출고방식/배송비/택배사) 기준으로 운영되니 이벤트 단위 주문마감/배송일이 불필요하다는 요청, 같은 이벤트 안에서도 상품마다 다른 뱃지(HOT/NEW/특가 등)를 붙이고 싶다는 요청 세 가지
- **모바일 전용 상품 목록 카드(`app/admin/events/[id]/page.tsx`)**: `EventProductRow`를 `sm:hidden`(모바일)과 `hidden sm:flex`(PC, 기존 그대로) 두 벌로 나눔. 모바일에서는 썸네일+상품명(1줄 ellipsis)을 맨 위에 두고, 판매가/원가/재고/판매수량·예상수익을 한 줄씩 세로로 쌓고, [숨기기]/[가격·원가 수정]/[제거] 버튼을 3열 그리드로 균등 배치 — PC 레이아웃(마크업 자체)은 손대지 않음
- **택배 이벤트 단순화**: `app/admin/events/[id]/page.tsx`, `new/page.tsx`, `page.tsx`(목록의 복제 폼)에서 `event.type === "PARCEL"`일 때 주문마감/배송일 입력을 화면에서 숨김. 택배 이벤트를 새로 만들거나 복제할 때는 두 값을 그냥 현재 시각으로 채워 넣음 — 택배는 `order-policy.ts`상 `ALWAYS_OPEN`이라 마감 정책 판단에 이 값을 애초에 안 쓰고, NOT NULL 컬럼이라 값 자체는 채워야 해서 택한 방식. 관리자 이벤트 목록 카드의 "마감 {시각}" 표시도 택배에서는 숨김
- **뱃지를 이벤트에서 상품으로 이동**: 기존엔 `events.badge`(NONE/SALE/HOT/NEW/RESERVE/DEADLINE) 하나로 이벤트 카드 전체에 뱃지를 붙였는데, 같은 이벤트 안에서도 상품마다 다르게 붙이고 싶다는 요청으로 `CatalogProduct`/`Product`에 `badge` 필드를 새로 추가하고 상품 관리(`/admin/products`) 화면에서 설정하도록 옮김. 고객 화면(홈 인기상품/카테고리/이벤트 상세/상품 상세)은 이제 상품 각각의 뱃지를 보여줌
  - **"특가" 뱃지와 "1시간 특가" 이벤트 기능은 완전히 분리**: 기존엔 `event.badge === "SALE"`이 (1) 홈 화면 "1시간 특가" 히어로 배너와 (2) 배송방식과 무관하게 마감이 지나면 즉시 주문을 막는 정책(`STRICT_DEADLINE`)을 동시에 결정했음. 이 두 기능은 이벤트 단위로 남아야 해서 `MarketEvent.badge`를 없애고 `MarketEvent.flashSale`(boolean)로 대체 — 관리자 이벤트 수정/등록 화면엔 "🔥 1시간 특가 이벤트로 지정" 체크박스(택배 제외)로 남음. 상품에 붙이는 표시용 "특가" 뱃지(`CatalogProduct.badge === "SALE"`)는 순수 노출용 라벨일 뿐 이 정책에는 전혀 영향을 주지 않음 — 이름만 같은 완전히 다른 값
  - 기존 이벤트에 저장돼 있던 뱃지 값(HOT/NEW 등)은 더 이상 쓰지 않음(요청에 따라 그대로 폐기)
- 관련 컴포넌트도 함께 정리: `components/admin/EventBadgePicker.tsx` → `components/admin/ProductBadgePicker.tsx`로 이름 변경(더 이상 이벤트가 아니라 상품에서 쓰므로)
- `tsc`/`build` 통과 확인. 모바일 레이아웃은 관리자 로그인이 필요해 이번 세션에서는 브라우저로 직접 확인하지 못함(기존과 동일한 한계) — Tailwind `sm:` 브레이크포인트 기반의 표준적인 반응형 분기라 코드 검토로 검증
- **DB 마이그레이션 필요**: 아래 SQL을 실 Supabase 프로젝트에 실행해야 함

```sql
-- 1) events: 표시용 뱃지 컬럼 제거, "1시간 특가" 여부만 남기는 flash_sale로 교체
alter table events add column if not exists flash_sale boolean not null default false;
update events set flash_sale = true where badge = 'SALE';
alter table events drop column if exists badge;

-- 2) products(카탈로그): 상품별 표시용 뱃지 컬럼 추가
alter table products add column if not exists badge text not null default 'NONE'
  check (badge in ('NONE', 'SALE', 'HOT', 'NEW', 'RESERVE', 'DEADLINE'));
```

**혼합 주문 자동 분리: 장바구니 그룹 표시 + 주문 생성 부분 실패 처리**
- 배경: 문고리/사다드림/택배가 섞인 장바구니에서 배송방식별로 결제수단이 다르고(사다드림은 무통장만) 주문이 자동으로 나뉘어야 한다는 요청 — 실제로 확인해보니 이 핵심 로직(체크아웃 그룹핑, 배송유형별 결제수단 제한, 배송유형 그룹별 독립 주문 생성/주문번호/조회/취소/알림/관리자 관리)은 이미 이전 세션(Epic 1 Phase 2)에서 구현돼 있었음. 이번엔 아직 없던 두 가지만 추가함: (1) 장바구니 화면도 체크아웃처럼 배송방식별로 미리 나눠 보여주기, (2) 주문 생성 도중 일부 그룹만 실패해도 이미 성공한 그룹까지 같이 날아가지 않게 하기
- **장바구니 배송방식별 그룹 표시(`components/Cart/CartView.tsx`)**: 문고리/사다드림/택배 섹션으로 먼저 나누고(▼ 문고리배송 (2) 처럼 개수 표시, 클릭하면 접기/펼치기) 그 안에서 회차(이벤트)별로 다시 묶어 보여줌 — 체크아웃(`CheckoutView.tsx`)과 완전히 같은 기준(상품별 deliveryType override 우선, 없으면 이벤트 타입)으로 나눠서 "장바구니에서 보이는 그룹"과 "체크아웃에서 실제로 나뉘는 그룹"이 항상 일치함
- **주문 생성 부분 실패 처리(`components/Checkout/CheckoutView.tsx`)**: 참고로 카드/카카오페이도 아직 실제 PG 연동 전이라 무통장입금처럼 관리자가 수동 확인하는 방식이라서, "결제 자체가 거절"되는 상황은 없고 "이 배송유형 그룹의 주문 생성이 실패"하는 상황만 있음(재고 경합, 네트워크/DB 오류 등). 예전엔 배송유형 그룹들을 하나의 `try/catch`로 묶어서 만들었는데, 그러면 뒤쪽 그룹 하나가 실패했을 때 이미 성공한 앞쪽 그룹까지 화면엔 "실패"로만 보이고 장바구니도 안 비워져서, 사용자가 다시 누르면 이미 만들어진 주문이 중복 생성될 위험이 있었음. 이제는 그룹마다 개별로 시도해서, 성공한 그룹은 바로 장바구니에서 지워 확정하고(다시 눌러도 중복 안 됨) 화면에 "이미 접수된 주문" 목록으로 계속 보여주고, 실패한 그룹의 상품만 장바구니에 남겨서 "남은 주문 다시 시도" 버튼으로 그 그룹만 재시도할 수 있게 함. 재시도해도 처음과 같은 `batchId`를 유지해서(컴포넌트 상태로 고정) 나중에 성공한 주문도 먼저 성공한 주문들과 계속 "한 번의 체크아웃"으로 묶여 보임(주문상세의 "함께 만들어진 다른 주문" 표시)
...
**카테고리 화면: 지난 회차 자동 숨김 + 배송방식/날짜 탭 고정**
- 배경: 문고리 회차가 배송일이 지나도 관리자가 "종료"를 따로 안 누르면 날짜 탭에 계속 남아있던 문제, 그리고 상품이 많아 아래로 스크롤하면 배송방식(문고리/사다드림/택배)·날짜 탭이 화면 위로 밀려 올라가 안 보이던 문제
- **지난 회차 자동 숨김(`lib/order-policy.ts`의 `isEventVisibleToCustomers`)**: 예전엔 관리자가 "종료" 버튼을 눌러야만(`status==='ended'`) 배송일 다음날부터 숨겨졌는데, 문고리/사다드림은 회차(날짜)마다 별도 이벤트라 종료를 안 눌러도 배송일이 지나면 자연히 끝난 회차다 — 그래서 상태와 무관하게 배송일 당일까지만 보이고 다음날 00:00부터 자동으로 숨기도록 바꿈. 다만 택배(PARCEL)는 상시 판매라 배송일이 "이 회차의 마지막 날"이라는 의미가 없어서(애초에 회차 개념이 없음) 이 자동 숨김 대상에서 제외했고, 관리자가 직접 종료를 눌렀을 때만 기존처럼 숨겨짐 — 안 그러면 새로 만든 택배 상품이 생성 다음날 바로 안 보이게 되는 심각한 버그가 될 뻔했음(택배는 등록 시 배송일을 입력받지 않고 생성 시각으로 자동 채워서, PARCEL도 똑같이 취급했으면 등록 하루 뒤 전부 사라질 뻔함)
- **탭 고정(`components/Category/CategoryView.tsx`)**: 배송방식 탭 + 날짜 탭을 한 덩어리로 묶어 `sticky`로 고정 — 헤더(`Header.tsx`)가 이미 `sticky top-0`로 71px 높이를 차지하고 있어서, 상품 상세 페이지의 상단 sticky 헤더와 같은 관례(`top-[71px]`)를 그대로 따라 헤더 바로 아래에 고정되게 함. 스크롤을 아무리 내려도 배송방식/날짜 탭이 항상 눌리는 위치에 남아있음
- `tsc`/`build` 통과 확인. 로컬 개발 서버(고객 화면이라 로그인 불필요, 실 프로덕션 Supabase 연결)에서 실제로 오늘(8/4) 기준 8/3 문고리 탭이 사라진 것, 택배 탭은 영향 없이 그대로 노출되는 것, 스크롤 시 탭 컨테이너가 헤더 바로 아래(71px)에 고정되는 것까지 자바스크립트로 좌표 확인함
- 마이그레이션 불필요(DB 스키마 변경 없음)

**내 주문 목록에 회차(배송방식+날짜) 라벨 추가**
- 배경: "내 주문" 목록에 주문번호/상태/상품명/금액만 보여서, 문고리·사다드림처럼 여러 회차가 있는 배송방식은 이 주문이 몇 월 며칠 회차인지 목록에서 바로 안 보여 헷갈린다는 요청
- `components/Orders/OrdersView.tsx`가 이벤트 목록을 같이 불러와(`listEvents`) 주문의 `eventId`로 조인, 주문번호 줄 바로 아래에 배송방식 뱃지(`EventTypeBadge`, 카테고리 화면과 같은 색상) + 날짜(`formatEventDateChip`, "8/4(월)" 형식)를 라벨로 붙임 — 예: "문고리배송 · 8/4(월)". 요청하신 대로 택배는 상시 판매라 회차 개념이 없어서 이 라벨을 안 붙임(택배 주문은 기존과 동일하게 표시)
- 회원 주문 목록/비회원 이름+PIN 조회 결과 양쪽 다 적용됨(둘 다 같은 `OrderRow` 컴포넌트를 씀)
- `tsc`/`build` 통과 확인. "내 주문" 화면은 실제 주문이 있는 계정으로 로그인해야 확인할 수 있는데 로그인 정보가 없어 이번에도 브라우저로 직접 열어보지는 못함(관리자 화면과 동일한 한계) — 코드 리뷰로 검증
- 마이그레이션 불필요(DB 스키마 변경 없음)

**상품 상세페이지 블록 에디터 전면 개편 (스마트스토어 스타일)**
- 배경: 상품 상세를 자유롭게 구성하기 어려웠던 것 개선 — 이미지/텍스트를 섞어 배치, 텍스트 서식(크기/굵기/색상), 이미지 교체, 동영상 삽입, 관리자 미리보기, 그리고 앞으로 배너/유튜브/버튼 같은 블록을 계속 추가할 수 있는 구조까지 요청받음
- **블록 구조(`types/index.ts`의 `ProductDetailBlock`)**: 판별 유니온(discriminated union) 하나가 전체 구조를 담당 — 새 블록 종류를 추가하려면 이 유니온에 멤버 하나, `DetailBlockEditor`에 편집 UI, `ProductDetailContent`에 렌더링 케이스만 각각 추가하면 됨(이미 확장 가능한 구조였고, 이번에 그 패턴을 그대로 따라 `video` 블록을 새로 추가). 예전엔 제목(`heading`)과 본문(`text`)이 분리된 블록이었는데, 이제 `text` 블록 하나에 크기(작게/보통/크게/아주 크게)·굵기·색상을 자유롭게 지정할 수 있어 굳이 나눌 필요가 없어짐 — `heading`은 기존에 저장된 상품을 그대로 보여주기 위한 레거시 타입으로만 남기고 새로 만들 때는 노출하지 않음
- **텍스트 서식(`components/admin/DetailBlockEditor.tsx`의 `TextBlockEditor`)**: 블록마다 크기 4단계 칩, "굵게" 토글, 색상 스와치(기본/진하게/브랜드/강조/파랑) — 렌더링은 `ProductDetailContent.tsx`가 그대로 반영
- **이미지 교체**: 기존엔 삭제 후 다시 추가해야 했는데, 각 사진 썸네일에 "교체" 버튼을 추가해 그 자리 URL만 새로 업로드한 파일로 바로 바꿀 수 있음(순서·다른 사진은 그대로 유지). 이 버튼은 처음엔 마우스 hover 때만 보이게 만들었다가, 문고리 배송 중 관리자를 모바일로 많이 쓴다는 점(터치 환경에선 hover가 안 먹힘)을 고려해 항상 보이도록 다시 고침
- **[수정] 사진 여러 장 한 번에 추가 시 블록 단위 → 장 단위로 재설계**: 처음엔 "사진 10장을 한 번에 올리면 그 10장이 통째로 이미지 블록 하나(공유된 1/2/3열 설정)"로 만들었는데, 사용자가 "사진을 한방에 추가하더라도 중간에 본문 넣을 수 있고, 그 안에서 내가 1열 2열 이렇게 골랐으면 좋겠다(블록별로 말고)"고 정정함 — 즉 업로드는 한 번에 하더라도 (1) 사진 사이사이에 텍스트를 끼워 넣을 수 있어야 하고 (2) 몇 장을 묶어서 몇 열로 보여줄지는 사진 묶음마다 자유롭게 골라야 한다는 뜻이었음. 그래서 `addPhotoBlock`을 "업로드한 사진 수만큼 낱장 블록을 각각 생성"하도록 바꾸고(`DetailBlockEditor.tsx`), 낱장 블록 옆에 **"▲ 이전 사진과 합치기"**(바로 위 이미지 블록과 합쳐 최대 3장까지 한 줄로 묶고 `columns`를 자동으로 묶은 장수에 맞춤) / **"낱장으로 나누기"**(합쳐진 블록을 다시 낱장으로 분해) 버튼을 추가해, 드래그 순서 변경 + 텍스트 블록 삽입과 조합하면 사진 묶음·열 구성을 원하는 대로 자유롭게 재구성할 수 있게 함. 블록 안에서 "+ 추가"로 사진을 더 넣는 것도 한 줄 최대 3장 한도에 맞춰 캡을 걸어둠(3장 채워지면 "+ 추가" 버튼 자체가 사라짐)
- **GIF**: 별도 처리 불필요 — 사진 블록의 업로드/드래그/붙여넣기가 전부 `image/*` MIME을 그대로 받아 `<img>`로 렌더링해서, GIF도 이미 애니메이션 그대로 동작함(확인만 하고 코드 변경 없음)
- **동영상 블록(신규 `video` 타입)**: "파일 업로드"와 "URL 입력" 두 방식 지원 — 업로드는 기존 사진과 같은 Storage 버킷에 올라가고(`lib/supabase/storage.ts`에 `uploadProductFile` 별칭 추가, 내부는 `uploadProductPhoto`와 동일), URL은 mp4 등 직접 재생 가능한 링크를 그대로 저장. 고객 화면은 `<video controls>`로 재생. 유튜브/비메오처럼 embed가 필요한 링크는 이번 범위가 아니고(요청하신 대로 "추후" 확장 대상), 지금 `video` 블록은 실제 영상 파일 전용
- **미리보기**: 편집기 상단에 "미리보기" 토글을 추가해 켜면 그 자리에서 `ProductDetailContent`로 실제 고객 화면과 똑같이 렌더링해서 보여줌(별도 페이지 이동 없이 바로 확인)
- `products.detail_blocks`가 이미 jsonb 컬럼이라 새 블록 종류·필드를 추가해도 스키마 변경이 필요 없음(마이그레이션 불필요)
...
**상품관리 폼 정리 + 택배 마감/배송예정 문구 숨김 + 특가 뱃지 문구 단순화 + 상세편집기 중간삽입**
- 상품관리 폼(`app/admin/products/page.tsx`): 이모지 입력칸/"상품 한 줄 설명" 칸 삭제, 판매정보·택배정보 섹션을 상세설명보다 위로 재배치(기존엔 기본정보 안에 상세설명이 끼어있어 판매가·배송정보를 보려면 아래로 더 스크롤해야 했음)
- 상품상세(`ProductDetailView.tsx`)/이벤트상세(`EventDetailView.tsx`): 택배(PARCEL)는 회차 개념이 없어 마감/배송예정일이 등록 시각으로 자동 채워진 의미 없는 값인데도 그대로 노출되던 것을 카테고리 날짜탭과 동일하게 숨김
- `EventBadgeTag`(`components/Badge.tsx`): SALE 뱃지에만 붙던 "🔥 1시간 특가" 특수 문구를 없애고 다른 뱃지처럼 `EVENT_BADGE_LABEL` 그대로("특가") 노출 — 실제 1시간짜리 타이머와 무관한 순수 표시용 뱃지인데 "1시간"이라는 문구가 오해를 일으킬 수 있었음
- `DetailBlockEditor.tsx`: 블록과 블록 사이(맨 위 포함)에 얇은 "+" 삽입 지점(`InsertPoint`)을 추가 — 누르면 텍스트/사진/동영상 미니 버튼이 펼쳐지고 바로 그 자리에 끼워 넣힘. 예전엔 새 블록이 항상 맨 아래에만 추가돼서 중간에 넣으려면 추가 후 드래그로 끌어올려야 했음
- `tsc`/`build` 통과 확인. 특가 뱃지 문구·택배 마감/배송예정 숨김은 로그인 없이 실 배포 데이터로 브라우저에서 직접 확인함("특가" 텍스트만 남은 것, "신안 흰다리 활새우" 택배 상품 페이지에 마감/배송예정 줄이 사라진 것). 상품관리 폼 재배치·중간삽입은 관리자 전용 화면이라 이번에도 로그인 없이는 직접 열어보지 못함(기존과 동일한 한계) — 코드 리뷰로 확인
- 마이그레이션 불필요(DB 변경 없음)

**재고관리 미사용 옵션 조합끼리 재고를 나눠쓰지 못하던 버그 수정**
- 배경: 재고 10개짜리 상품에 옵션(예: 110g/390g)을 달았는데 옵션값에는 "재고관리"를 켜지 않은 상태(재고는 상품 전체로만 관리하고 싶은 흔한 케이스) — 이 경우 옵션별로 각 10개씩 담을 수 있고, 결제 직전에야 재고부족으로 튕겨나가고, 장바구니에서 한 옵션 줄의 수량을 +/-하면 다른 옵션 줄까지 같이 바뀌는 것처럼 보이는 버그 리포트
- **근본 원인**: `lib/product-options.ts`의 재고 계산이 옵션값 조합을 식별하는 "콤보 키"가 빈 문자열(재고관리 옵션 미사용)일 때, "이미 다른 줄에서 쓰인 양"을 무조건 0으로 취급했음. 원래 이 0 처리는 "재고관리 대상 조합 자체가 없으니 조합별 제한은 없다"는 뜻으로 만들어진 것인데, 상품 전체가 공유하는 `product.stock`까지 같이 0 취급돼버려서 옵션 조합마다 재고 10개를 각각 독립적으로 다 쓸 수 있는 것처럼 계산됐음(실제로는 여러 조합이 재고 10개를 나눠 가져야 함)
- **수정**: "상품 전체 재고를 옵션 조합 전체가 나눠 쓰는 것"과 "옵션 조합별(콤보) 재고를 그 조합끼리만 나눠 쓰는 것"을 완전히 분리해서 계산하는 `remainingForCombo`(장바구니/상품상세에서 "지금 더 담을 수 있는 최대치")·`findOverStockLines`(체크아웃 최종 검증 — 상품 합계와 조합 합계를 각각 따로 집계해 넘는 라인만 골라냄) 두 헬퍼로 재설계, 기존 `maxQtyForSelection`/직접 계산 로직을 쓰던 `CartView.tsx`/`ProductDetailView.tsx`/`CheckoutView.tsx` 세 곳 모두 교체
- `tsc`/`build` 통과 확인. 실제 Supabase 로그인 세션 없이도 재현 가능한 순수 계산 로직이라, 리포트된 시나리오(재고10 + 재고관리 미사용 옵션 2개)를 그대로 코드로 재구성해 `remainingForCombo`/`findOverStockLines`를 직접 호출하는 회귀 테스트로 확인함(다른 줄 사용량 반영, 합계가 재고를 넘는 조합 검출, 정확히 재고만큼일 때 통과, 단일 옵션 초과 케이스 회귀 방지 총 5가지 모두 통과) — 실 배포 데이터는 건드리지 않기 위해 브라우저 장바구니 조작으로는 검증하지 않음
- 마이그레이션 불필요(DB 변경 없음, 순수 프론트 계산 로직 수정)

**대시보드 "오늘 매출" + 배송지 다중 저장/선택 + 알림 보관기간 관리자 설정**
- 배경: 다음 작업(TODO) 목록에 있던 3가지 항목을 한 번에 진행 — (1) 대시보드에 "오늘 매출" 개념 자체가 아직 없어서 새로 추가하며 처음부터 입금대기 금액을 제외하도록 설계, (2) 배송지가 회원당 하나만 편집 가능해서 자주 쓰는 배송지를 매번 덮어써야 했음(백엔드는 `addresses` 테이블이 원래부터 여러 행 + `is_default`를 지원하도록 설계돼 있었지만 UI가 하나만 쓰도록 만들어져 있었음), (3) 알림 보관기간이 `lib/notification-state.ts`에 30일로 하드코딩(관리자 설정으로 뺄 걸 염두에 두고 상수로만 분리해뒀던 상태)
- **오늘 매출**(`app/admin/page.tsx`): 오늘 생성된 주문 중 `wait`(입금대기)·`cancelled`·`refunded` 상태를 제외하고 합산하는 카드를 대시보드 타일 위에 신규 추가 — 발주확인~반려 사이는 실제로 입금이 확인된 돈이라 그대로 포함
- **배송지 다중 저장/선택**: `lib/data.ts`에 `deleteAddress`(기본 배송지를 지우면 남은 것 중 하나를 자동으로 새 기본으로 승격)와 `setDefaultAddress`(다른 행의 `is_default`를 먼저 내리고 지정한 행만 켬) 신규 추가, `updateAddress`가 `isDefault` patch도 반영하도록 수정(기존엔 이 필드만 빠져있었음). 마이페이지(`components/MyPage/MyPageView.tsx`)에 "배송지 관리" 섹션을 새로 만들어 저장된 배송지를 전부 목록으로 보여주고 각각 수정/삭제/기본설정 + "새 배송지 추가"를 지원. 체크아웃(`components/Checkout/CheckoutView.tsx`)은 로그인 회원이면 저장된 배송지 목록을 선택 버튼으로 보여주고(기본이 자동 선택됨), "새 배송지 입력"을 고르면 빈 폼으로 전환 — 고른 뒤에도 필드를 직접 고쳐서 "이번 주문만 다르게" 보낼 수 있음. 주소 테이블 RLS(`profile_id = auth.uid()`, `for all`)가 이미 CRUD 전체를 허용하고 있어서 스키마/RLS 변경은 필요 없었음
- **알림 보관기간 관리자 설정**: `store_settings`에 `notification_retention_days`(기본 30) 컬럼 추가, `isWithinRetention()`이 하드코딩 상수 대신 인자로 받도록 변경. 관리자 설정(`/admin/settings`)에 입력란 추가, 고객 알림 화면과 헤더 안읽음 배지 둘 다 설정값을 읽어와 필터링에 사용
- **검증**: 이 3가지는 실 배포 데이터로는 테스트할 수 없어서(주소/주문 생성이 실제 고객 데이터를 오염시킴), `.env.local`을 건드리지 않고 `NEXT_PUBLIC_SUPABASE_URL`을 빈 값으로 덮어써서 별도 포트(3001)에 mock 모드 전용 개발 서버를 띄우고 브라우저로 전체 플로우를 직접 확인함: 회원가입 없이 `localStorage`에 테스트 프로필/배송지를 직접 심어 마이페이지에서 배송지 추가·기본설정 전환·삭제 시 자동 승격까지 확인, 체크아웃에서 배송지 선택 시 필드가 정확히 바뀌는지 확인, 실제로 상품을 담아 주문을 접수하고 관리자 화면에서 입금대기→입금확인으로 상태를 바꾸며 "오늘 매출"이 0원→실제 금액으로 정확히 바뀌는 것도 확인, 관리자 설정에서 보관기간을 14일로 저장한 뒤 5일 전/20일 전 알림을 심어 20일 전 알림만 고객 화면에서 사라지는 것도 확인. **이 과정에서 체크아웃 폼 수정 중 실수로 전화번호 입력란을 통째로 지워버린 회귀를 발견해 즉시 수정함** — tsc/build만으로는 못 잡는 종류의 버그라 브라우저 검증이 실제로 값을 했음
- **마이그레이션 필요** — 실 Supabase 프로젝트에 아래 SQL 실행 필요(안 해도 배송지 다중저장/오늘 매출은 그대로 동작하고, 실행 전까지는 관리자 설정에서 알림 보관기간 저장만 실패함):
  ```sql
  alter table store_settings add column if not exists notification_retention_days integer not null default 30;
  ```

**마이페이지: 배송지 관리를 "정보 수정" 안으로 이동**
- 바로 위 항목에서 배송지 관리를 항상 펼쳐진 별도 카드로 만들었는데, 요약 화면에서도 계속 보여서 어수선하다는 피드백 — 기본 정보/비밀번호 변경과 같은 "정보 수정" 토글 안, 기본 정보 바로 아래로 옮김. 정보 수정을 취소하면 배송지 쪽에 열려있던 편집 폼도 같이 닫히도록 정리
- 마이그레이션 불필요(DB 변경 없음, 순수 레이아웃 변경)

**크리티컬 버그: 장바구니가 계정 간에 공유되던 문제 수정**
- 배경: 로그아웃 후 다른 계정으로 로그인해도 이전 계정이 담아둔 장바구니가 그대로 보인다는 리포트. 원인은 `lib/cart-context.tsx`가 로그인 여부/계정과 무관하게 항상 고정된 localStorage 키(`bogle_cart`) 하나만 썼기 때문 — 브라우저 하나를 여러 명이 돌아가며 로그인하면(같은 기기를 가족이 같이 쓰는 등) 다른 사람의 장바구니를 그대로 물려받는 구조였음
- **수정**: 장바구니를 신원별로 완전히 분리 저장하도록 재설계 — 비회원은 `bogle_cart_guest`, 로그인 계정은 `bogle_cart_<profileId>` 하나씩. `CartProvider`가 `useAuth()`로 로그인 상태를 구독해서, 신원이 바뀔 때(로그인/로그아웃/계정 전환)마다 그 신원의 저장소로 전환한다. 딱 하나 예외로 "비회원 → 로그인/회원가입" 전환일 때만 비회원 장바구니를 그 계정의 기존 장바구니에 합치고(수량 합산) 비회원 저장소는 비운다 — "담아두고 로그인해도 그대로 남아있어야 한다"는 기존 기대는 유지하면서, 로그아웃/다른 계정 로그인 시에는 병합 없이 그 신원 고유의 장바구니로 완전히 바꿔치기한다
- **기존 사용자 장바구니 보존**: 이 배포 전까지 모두가 공유하던 옛 키(`bogle_cart`)에 남아있는 데이터는 잃어버리면 안 되므로, 처음 하이드레이션 시점에 지금 신원의 새 저장소가 비어있으면 옛 키를 한 번만 읽어 그 신원 몫으로 넘겨주고 옛 키는 지운다(전 사이트에 걸쳐 딱 한 번만 소비됨)
- **검증**: mock 전용 개발 서버(포트 3001, 위 항목과 동일한 방식)에서 두 개의 테스트 계정을 만들어 실제 로그인/로그아웃 UI로 재현 — 비회원으로 담기 → 로그인(계정A) → 장바구니에 그대로 남아있음(병합) 확인 → 로그아웃 → 장바구니 0개 확인 → 계정B로 로그인 → 계정A의 장바구니가 전혀 안 보임(빈 장바구니) 확인 → 계정B에 다른 상품 담기 → 로그아웃 → 계정A로 재로그인 → 계정A가 전에 담아뒀던 장바구니가 그대로(계정B의 상품 섞임 없이) 복원되는 것까지 확인. `localStorage`를 직접 조회해 `bogle_cart_guest`/`bogle_cart_<id>` 각각의 내용이 항상 정확히 분리돼 있는 것도 대조함
- 마이그레이션 불필요(로컬 저장소 구조만 변경, DB 변경 없음)

**관리자 화면: 마감 지난 이벤트를 자동으로 "종료" 쪽으로 분류**
- 배경: 관리자 이벤트 관리 목록이 순수하게 `status` 필드로만 진행중/종료를 나눠서, 마감이 이미 지난 문고리/사다드림 회차도 관리자가 직접 "종료"를 누르기 전까진 계속 "진행중"에 남아있었음(스크린샷으로 리포트) — 마감이 지났으면 관리자 입장에선 이미 끝난 회차인데 목록엔 계속 진행중으로 잡혀서 어수선함
- **수정**: `lib/order-policy.ts`에 `isEventDeadlinePassed`/`isEventAdminEnded` 두 헬퍼를 새로 추가 — 마감(`deadlineAt`)이 지났으면(택배는 마감 개념이 없어 제외) 관리자 화면 분류상 "종료"로 취급한다. 이걸 이벤트 관리 목록(`app/admin/events/page.tsx`)의 진행중/종료 탭·정렬·회색처리와, 운영 메인 대시보드(`app/admin/page.tsx`)의 "진행중 이벤트"/"종료 이벤트" 타일 카운트 양쪽에 똑같이 적용해서 두 화면 숫자가 항상 일치하게 함
- **실제 주문 차단과는 분리**: 이 자동 분류는 어디까지나 관리자 목록을 정리하기 위한 것으로, 실제 주문 가능 여부(`isEventOrderable`)는 전혀 건드리지 않았음 — 문고리(SOFT_DEADLINE)는 설계상 마감이 지나도 재고가 있으면 계속 주문을 받게 돼 있어서, 마감 지났다고 자동으로 `status`를 DB에 `ended`로 써버리면 그 주문 유입 경로를 조용히 막아버리는 부작용이 생김. 그래서 마감만 지나고 관리자가 아직 실제로 "종료"를 안 누른 이벤트는 뱃지를 "마감"(회색, 종료 탭에 포함되지만)으로, 관리자가 직접 종료 버튼을 눌러 실제로 DB에 반영된 경우만 "종료됨"(재시작 버튼 노출)으로 구분해서 보여줌 — "종료" 버튼을 누르면 그제서야 진짜로 주문이 막힘
- **검증**: mock 서버에서 문고리 이벤트 하나의 마감을 어제로 조작해 목록에서 자동으로 "마감" 뱃지 + 종료 탭으로 이동하고 대시보드 타일 카운트도 같이 바뀌는 것을 확인, 그 상태에서 "종료" 버튼을 눌러 실제로 `status`가 `ended`로 저장되고 뱃지가 "종료됨" + 버튼이 "재시작"으로 바뀌는 것까지 확인
- 마이그레이션 불필요(DB 변경 없음, 관리자 화면 분류 로직만 변경)

**이벤트 상품별 개별 "마감" 기능**
- 배경: 위풍당당쭈꾸미·양고기처럼 예약상품이라 발주 마감이 나머지 상품보다 먼저 오는 경우(목요일 오전 마감, 배송은 금요일이지만 재고 있는 다른 상품들은 금요일까지 계속 주문받음)를 지원할 방법이 없어서, 이벤트 안 상품 하나만 먼저 주문을 막을 수 있는 "마감" 버튼을 요청받음
- **설계**: 기존 `visible`(노출여부) 필드와 같은 테이블·같은 패턴이지만 의미는 반대 — `visible=false`는 고객 화면에서 아예 숨기고, `closed=true`는 화면엔 계속 보이되(투명성) 주문만 막고 "마감" 표시가 붙음. 재고(`product.stock`)와는 완전히 독립된 관리자 수동 게이트라, 마감시켜도 실제 재고 수치는 건드리지 않음(관리자 화면의 재고 파악에 영향 없음). 이벤트 전체 종료(`events.status`)와도 별개 개념이라 이벤트는 계속 진행 중이어도 상품 하나만 마감시킬 수 있음
- **스키마**: `event_products.closed`(boolean, 기본 false) 신규 컬럼. `create_order` RPC에 마감된 리스팅이 섞여 있으면 주문 전체를 거부하는 예외 추가(트랜잭션 롤백)
- **관리자(`app/admin/events/[id]/page.tsx`)**: 상품 목록의 각 리스팅에 "마감"/"마감 해제" 버튼 추가(숨기기 옆) — 누르면 그 리스팅만 `closed`를 토글하고, 마감시키는 순간 그 리스팅을 목록 맨 아래로 자동 재정렬(`sort_order` 활용). "마감" 배지도 "숨김" 배지와 나란히 표시. 마감 해제 시에는 순서를 되돌리지 않음(관리자가 필요하면 ▲▼로 직접 조정)
- **고객 화면 5곳 반영**: 이벤트 전체 마감(`isEventOrderable`)과 상품별 마감(`product.closed`)을 항상 OR로 합쳐서 판단하도록 통일 — `ProductGridCard`(홈/카테고리 그리드, 이미 있던 `closed` prop과 결합), `EventDetailView`(이벤트 상세 상품목록, "마감" 배지로 대체), `ProductDetailView`(상품상세, "마감된 상품이에요" 안내 + 담기 버튼 비활성 — 이벤트 마감 문구와 구분), `CartView`(장바구니 줄, 수량조절 대신 "마감" 표시로 더 이상 못 늘리게), `CheckoutView`(주문 직전 마감 상품이 섞여 있으면 하드 차단, 기존 이벤트 마감 체크와 동일한 패턴으로 추가)
- **검증**: `tsc`/`build` 통과 확인. mock 서버(포트 3001)에서 실제 관리자 UI로 "7/28 문고리배송" 이벤트의 "유정란 20구" 리스팅에 마감 버튼을 눌러 배지 표시 + 맨 아래로 이동 + 버튼이 "마감 해제"로 바뀌는 것 확인, 이벤트 상세 페이지에서 그 상품만 "마감" 표시로 바뀌고 나머지 두 상품은 정상 주문 가능한 것 확인, 상품 상세 페이지에서 "마감된 상품이에요" 안내와 비활성 버튼 확인, 장바구니에 담긴 상태에서 "마감" 표시로 바뀌어 수량을 못 늘리는 것 확인, "마감 해제"로 정상 복귀되는 것까지 확인. 체크아웃의 하드 차단은 Daum 주소검색 팝업이 자동화 브라우저에서 막혀 있어 실제 제출까지는 못 눌러봤지만, 기존에 이미 검증된 `closedGroups`(이벤트 마감 차단) 코드와 완전히 동일한 패턴이라 안전하다고 판단
- **마이그레이션 필요** — 실 Supabase 프로젝트에 아래 SQL 실행 필요(안 해도 다른 기능은 그대로 동작하고, 실행 전까지는 "마감" 버튼을 눌러도 저장만 실패함):
  ```sql
  alter table event_products add column if not exists closed boolean not null default false;

  -- create_order RPC는 시그니처는 그대로고 본문만 바뀌므로 drop 없이 재정의 가능:
  -- (schema.sql의 create_order 최신 정의 전체를 SQL 에디터에서 실행)
  ```

**유통기한/소비기한 필드 추가**
- 배경: 상품마다 유통기한 표기가 맞는지 소비기한 표기가 맞는지 다르고(육류/수산물은 소비기한, 가공식품은 유통기한이 흔함), 값도 "2026-12-31"처럼 날짜일 수도 "제조일로부터 7일"처럼 기간일 수도 있어서 요청받은 대로 라벨(유통기한/소비기한 선택)과 값(자유 텍스트) 두 필드로 분리해 구현
- `products.expiry_label`(유통기한/소비기한 중 선택, null 가능)/`expiry_value`(자유 텍스트) 컬럼 추가. 관리자 상품관리(`/admin/products`) 기본 정보 섹션에 원산지/중량/보관법/조리법 옆에 드롭다운+입력란 추가 — 라벨과 값이 둘 다 채워졌을 때만 저장(하나만 있으면 고객 화면에 보여줄 방법이 없어서). 상품 상세 정보 표에 다른 항목들과 같은 자리에 표시(라벨을 못 정했으면 "유통기한"으로 기본 표시하되, 값이 없으면 이 줄 자체가 안 뜸)
- **검증**: `tsc`/`build` 통과 확인. mock 서버(포트 3001)에서 관리자 상품관리로 실제 상품에 "소비기한 / 제조일로부터 5일"을 입력·저장하고, 상품 상세 페이지 정보 표에 그대로 표시되는 것 확인
- **마이그레이션 필요** — 실 Supabase 프로젝트에 아래 SQL 실행 필요(안 해도 다른 기능은 그대로 동작하고, 실행 전까지는 이 필드를 입력해도 저장만 실패함):
  ```sql
  alter table products add column if not exists expiry_label text
    check (expiry_label is null or expiry_label in ('유통기한', '소비기한'));
  alter table products add column if not exists expiry_value text;
  ```

**재고 임박 수치 노출 방식 변경 + 옵션 필수 설정 안내 강화**
- 배경: 상품 상세에서 옵션을 담을 때 "최대 10개까지 담을 수 있어요"처럼 남은 재고 수를 항상 미리 보여주고 있었는데, 이게 너무 직관적으로 "재고가 얼마 안 남았다"는 인상을 줘서 "잘 안 나가나?" 싶은 느낌을 줄 수 있다는 피드백. 재고 자체를 숨기되, 실제로 한도를 넘겨 담으려는 시도가 있을 때만("+" 버튼을 눌렀는데 재고가 없을 때) 그 자리에서 안내하는 방식으로 변경
- **수정**: `components/QtyControl.tsx`(그리드/장바구니/이벤트상세 공용), `components/Product/ProductDetailView.tsx`(옵션 조합 담기 목록 + 옵션 없는 상품의 수량 스테퍼) 세 군데 모두 동일한 패턴으로 변경 — "+" 버튼을 항상 눌리게 두고, 한도(`max`/`remaining`/`cap`)에 도달한 상태에서 누르면 수량은 그대로 두고 화면 하단에 "최대 N개까지만 구매할 수 있어요." 토스트만 띄운다. 재고가 아예 0이면(품절) 기존처럼 "품절" 표시로 완전히 막힘 — 이 부분은 변경 없음
- **옵션 "필수 담기 안 됨" 버그 조사**: "옵션 선택 안 하고 '이 옵션 담기'를 눌러도 담아진다"는 리포트를 조사한 결과, 코드상 버그가 아니라 그 상품의 옵션 그룹이 관리자 화면에서 "필수" 체크가 꺼진 상태였던 것으로 확인됨(`필수` 기본값이 `false`인 건 의도된 설계 — 예전에 기본값이 `true`였다가, 관리자가 깜빡하고 안 끄면 고객이 옵션을 못 골라 담기 자체가 막히는 정반대 문제가 있어서 지금 방식으로 바꾼 이력이 있음, `components/admin/ProductOptionEditor.tsx` 주석 참고). 실제로 필수를 켠 옵션 그룹으로 재현해보니 선택 없이는 "이 옵션 담기" 버튼이 비활성화되고 "옵션을 모두 선택해 주세요." 안내가 정상적으로 뜸(맛/사이즈처럼 반드시 골라야 하는 옵션은 상품관리에서 "필수" 체크가 필요) — 같은 실수가 반복되지 않도록 관리자 상품관리 화면의 "필수" 체크박스 옆에 "필수 체크를 안 하면 고객이 이 옵션을 하나도 안 골라도 '담기'가 돼요" 안내문을 추가
- **검증**: `tsc`/`build` 통과 확인. mock 서버(포트 3001)에서 재고 3개짜리 상품으로 수량을 3개까지 올리고 4번째 "+"를 눌러 "최대 3개까지만 구매할 수 있어요." 토스트가 뜨고 수량은 3에 멈추는 것 확인, 필수 옵션 그룹을 만들어 선택 없이 담기 버튼이 비활성화되는 것 확인, 관리자 화면에 새 안내문이 표시되는 것 확인
- 마이그레이션 불필요(DB 변경 없음, 프론트 UX 변경 + 안내문 추가만)

**관리자 이벤트 목록: 자동 "마감" 이벤트에도 재시작(마감시간 연장) 버튼 추가**
- 배경: 관리자가 직접 "종료"를 눌러 종료한 이벤트는 "종료됨" 뱃지 + "재시작" 버튼으로 되돌릴 수 있는데, 마감시간만 지나서 자동으로 "마감"으로 분류된 이벤트(DB상 `status`는 여전히 `open`)는 "종료" 버튼만 있고 "재시작"이 없어서 일관성이 없다는 리포트. 이 상태에서 "재시작"이 `status`를 되돌리는 기존 방식으로는 동작할 게 없다(이미 `open`이라) — 진짜로 되돌리려면 지난 마감시간 자체를 미래로 늦춰야 함
- **수정**: `app/admin/events/page.tsx`에 마감된(자동/수동 구분 없이) 이벤트 카드에 "재시작" 버튼을 항상 노출하되, 동작을 분기 — 수동 종료(`status=ended`)는 기존처럼 `status`만 되돌리고, 자동 마감(마감시간만 지남)은 새로 만든 `ExtendDeadlineForm`이 펼쳐져서 지금 시각 기준 "+1시간/+3시간/+6시간/+1일/+3일" 버튼이나 직접 날짜/시간을 골라 마감시간을 연장할 수 있음. 연장하면 그 즉시 마감시간이 미래가 돼서 목록에서 자동으로 "진행중"으로 돌아감(별도 상태 플래그 없음, `isEventAdminEnded`가 실시간 계산)
...
**상품별 주문 마감시간 예약**
- 배경: 지금은 이벤트 단위로만 주문 마감시간을 정할 수 있는데, 위풍당당쭈꾸미·양고기처럼 개별 상품만 먼저 발주를 마감해야 하는 경우 관리자가 그 시각에 맞춰 직접 "마감" 버튼을 눌러줘야 했음(이미 있는 수동 마감 기능). 이번 요청은 그 시각을 미리 예약해두면 자동으로 마감되고, 재고가 추가로 확보되면 다시 열 수 있어야 한다는 것 — 기존 수동 "마감" 토글을 대체하는 게 아니라 "예약된 시각이 되면 자동으로 같은 효과를 낸다"는 별도 메커니즘으로 설계해, 두 방식이 항상 공존함
- **설계**: `event_products.order_deadline_at`(nullable timestamptz) 추가. 기존 `closed`(즉시 수동 토글)와 신규 `orderDeadlineAt`(예약된 자동 마감)을 하나로 합쳐 판단하는 `lib/order-policy.ts`의 `isListingOrderable(product)` 헬퍼를 신설하고, 상품 주문 가능 여부를 확인하던 5개 클라이언트 화면(`ProductGridCard`/`EventDetailView`/`ProductDetailView`/`CartView`/`CheckoutView`)과 서버 검증(mock의 `assertStockAvailable`, Supabase의 `create_order` RPC)을 전부 `product.closed` 직접 체크에서 이 헬퍼로 교체 — 마감 판정 로직이 한 곳에만 있으면 되도록 통일
- **관리자(`app/admin/events/[id]/page.tsx`)**: 각 리스팅에 "발주 마감 예약" datetime-local 입력란 추가(`OrderDeadlineField`, blur 시 즉시 저장 — 기존 옵션재고 입력란과 동일한 패턴). 예약 시각이 지나면 라벨 옆에 "(지남 — 자동 마감됨)" 빨간 안내가 뜨고 "마감" 배지도 자동으로 붙음(수동 마감 버튼의 라벨 자체는 건드리지 않음 — 수동/자동은 별개 상태이므로). "예약 해제" 버튼으로 예약 자체를 없애면(재고 확보 시) 즉시 다시 주문 가능 상태로 돌아감, 미래 시각으로 늦춰도 동일하게 재오픈됨
- **검증**: `tsc`/`build` 통과 확인. mock 서버(포트 3001)에서 "유정란 20구" 리스팅에 24시간 전 과거 시각을 예약 저장 → 관리자 화면에 "마감" 배지 + "(지남 — 자동 마감됨)" 표시, 고객 상품상세("마감된 상품이에요" 안내), 이벤트 상세(그 상품만 "마감", 나머지 두 상품은 정상 주문 가능), 장바구니(수량조절 대신 "마감" 표시) 네 화면 모두 정상 반영 확인. "예약 해제" 클릭 → 즉시 정상 주문 가능 상태로 돌아가는 것까지 확인
- **버그 수정**: 검증 중 `OrderDeadlineField`의 로컬 `value` state가 저장 후 갱신되는 `product.orderDeadlineAt`을 다시 반영하지 못해(최초 렌더 시점 값으로 고정) 두 번째 blur부터 저장 여부 비교가 어긋날 수 있는 문제를 발견, `useEffect`로 prop 변경 시 `value`를 재동기화하도록 수정
- **마이그레이션 필요** — 실 Supabase 프로젝트에 아래 SQL 실행 필요(안 해도 다른 기능은 그대로 동작하고, 실행 전까지는 "발주 마감 예약" 입력이 저장만 실패함):
  ```sql
  alter table event_products add column if not exists order_deadline_at timestamptz;

  -- create_order RPC는 시그니처는 그대로고 본문만 바뀌므로 drop 없이 재정의 가능:
  -- (schema.sql의 create_order 최신 정의 전체를 SQL 에디터에서 실행 —
  --  마감 판정 조건이 `where ep.closed`에서
  --  `where ep.closed or (ep.order_deadline_at is not null and ep.order_deadline_at <= now())`로 바뀜)
  ```

**상품 할인 정책 (수량 기반)**
- 배경: 마지막으로 남아있던 6개 개선 요청 중 하나 — "2개 이상 구매 시 1,000원 할인", "개당 500원 할인", "3개 구매 시 2,000원 할인", "N+1", "묶음 할인"처럼 다양한 수량 기반 할인을 상품마다 설정하고 싶다는 요청. 가격 계산이 상품상세/장바구니/체크아웃/주문스냅샷 전체에 걸쳐 있어 구조를 잘못 잡으면 나중에 손대기 어려워, 진행 전 "상품 하나에 할인을 몇 개까지 걸 수 있어야 하는지"(동시 중첩 허용 여부)를 사용자에게 확인 — **상품당 하나만**(추천안)으로 확정 후 진행
- **설계**: `products.discount`(nullable jsonb, 카탈로그 단위 — origin/badge와 같은 성격) 하나에 `{type, ...params}` 모양으로 저장하는 태그된 유니온(`ProductDiscount`, `types/index.ts`)으로 설계 — 종류가 늘어도 컬럼을 계속 추가하지 않고 유니온 케이스만 늘리면 되는 구조. 지금 구현한 4종:
  - `qty_threshold`(`minQty`, `amountOff`): N개 이상 구매 시 총액에서 정액 할인
  - `per_unit`(`amountOff`): 수량과 무관하게 개당 정액 할인
  - `n_plus_1`(`buyQty`): N개 살 때마다 1개 무료
  - `bundle`(`bundleQty`, `bundlePrice`): N개 묶음마다 정가 대신 지정 가격
  - 계산 로직은 `lib/shipping.ts`(택배 배송비)와 완전히 같은 패턴으로 새로 만든 `lib/discount.ts`에 모음 — 카탈로그 상품 단위로 옵션조합이 다른 여러 장바구니/주문 줄을 묶어(같은 상품이면 옵션이 달라도 합산) 그 상품에 담은 총 수량 기준으로 딱 한 번 계산한다(최소구매수량 합산·배송비 계산과 동일한 이유). 단가는 묶인 줄들의 평균 단가(subtotal/qty)를 써서 n_plus_1/bundle처럼 "1개 값"이 필요한 계산에도 옵션 추가금이 섞여 있어도 합리적으로 근사
- **관리자(`app/admin/products/page.tsx`)**: 카탈로그 상품 폼의 "판매 정보" 섹션에 "수량 할인" select(할인 없음/4종) + 종류별 입력란(예: qty_threshold는 최소수량+할인금액) 추가, 저장 전 미리보기 문구(`describeDiscount`)로 관리자가 바로 확인 가능. 상품 목록 카드 요약 줄에도 할인 문구를 덧붙임
- **고객 화면**: 상품상세(`ProductDetailView.tsx`)는 가격 아래 할인 정책을 안내 배지로만 보여줌(수량별 실시간 재계산은 안 함 — 배송비 안내와 같은 수준의 정적 안내). 실제 할인 반영은 장바구니(`CartView.tsx`)·체크아웃(`CheckoutView.tsx`)에서 카탈로그 상품별로 묶어 "할인 -X원" 줄로 보여주고 최종 결제금액에서 차감 — 배송비 표시와 대칭되는 자리(배송비는 더하고 할인은 뺌)
- **주문 스냅샷**: `orders.discount_total`(shippingFee와 대칭 컬럼) 추가 — 체크아웃에서 클라이언트가 계산한 할인액을 그대로 스냅샷으로 저장(shippingFee와 동일한 신뢰 모델: 이 앱은 결제를 관리자가 수동으로 확인하는 소규모 공동구매라 서버가 가격 공식을 처음부터 다시 계산해 검증하지 않고, client가 계산한 total/shippingFee를 그대로 저장해온 기존 패턴을 그대로 따름 — 재고/최소구매수량/마감 여부처럼 "허용되는지"만 서버가 별도로 재검증). `create_order` RPC에 `p_discount_total` 파라미터 추가
- **검증**: `tsc`/`build` 통과 확인. mock 서버(포트 3001)에서 "유정란 20구"에 "2개 이상 구매 시 1,000원 할인"을 설정하고 관리자 폼 미리보기 문구 확인, 상품 상세 페이지에 할인 안내 배지 노출 확인, 수량 3개를 장바구니에 담아 "할인 -1,000원"(정액이라 3개여도 1,000원 그대로)과 상품 3개 합계 23,700원에서 22,700원으로 차감되는 것 확인, 체크아웃 화면의 그룹별/전체 요약에도 동일하게 반영되는 것 확인. 다만 체크아웃의 실제 "주문하기" 클릭까지는 Daum 주소검색 팝업이 자동화 브라우저에서 막혀 있어 못 눌러봤음 — 체크아웃 화면에 표시되는 할인액과 `placeOrder()`가 `createOrder`에 넘기는 `discountTotal`이 같은 `groupDiscountAmount()` 호출 결과를 쓰므로(화면에 보이는 값과 실제 저장되는 값이 항상 같은 코드 경로), 화면 검증으로 충분하다고 판단(이전 세션의 상품별 마감 기능 검증 때와 동일한 판단 근거)
- **마이그레이션 필요** — 실 Supabase 프로젝트에 아래 SQL 실행 필요(안 해도 다른 기능은 그대로 동작하고, 실행 전까지는 할인 정책을 설정해도 저장만 실패함):
  ```sql
  alter table products add column if not exists discount jsonb;
  alter table orders add column if not exists discount_total integer not null default 0 check (discount_total >= 0);

  -- create_order RPC는 시그니처는 그대로고 파라미터 하나(p_discount_total)만
  -- 끝에 추가되므로 drop 없이 재정의 가능:
  -- (schema.sql의 create_order 최신 정의 전체를 SQL 에디터에서 실행)
  ```
...
- [x] ~~대시보드 "오늘 매출"이 입금대기(미확인) 주문까지 합산~~ → 완료(위 "완료된 기능" 참고)
...
- [x] ~~배송지 다중 저장/선택 UI (현재는 최근 1건만 자동 채움)~~ → 완료(위 "완료된 기능" 참고)
- [x] ~~알림 보관 기간(현재 하드코딩 30일)을 관리자가 조정할 수 있는 설정 화면~~ → 완료(위 "완료된 기능" 참고)
- [ ] 주소검색 결과 기반 문고리배송 가능 단지 자동 판별/배송 가능 지역 체크 (아파트명 컬럼이 이미 있어서 자연스러운 확장)
- [ ] ⚠️ **(배포 차단)** 실제 Supabase 프로젝트에 `store_settings.notification_retention_days` 마이그레이션 미적용 — 안 해도 다른 기능은 그대로 동작하지만, 관리자 설정에서 알림 보관기간을 저장하면 에러가 남. 대화에서 안내한 마이그레이션 SQL을 SQL 에디터에서 반드시 먼저 실행할 것
...
| `event_products` | id, event_id, product_id(→products), price, delivery_type, visible, closed, order_deadline_at, sort_order | **이벤트별 상품 리스팅** — 옛 `products` 테이블이 하던 역할(이벤트에 속한 판매 정보)을 그대로 가져오되 내용은 `product_id`로 카탈로그를 참조. `price`/`delivery_type`/`visible`/`closed`는 리스팅마다 독립적이라, 같은 카탈로그 상품도 회차별로 다른 가격·노출 여부·마감 여부를 가질 수 있음(재고는 없음 — `products.stock` 참고, Epic 1 Phase 3 전엔 여기 있었음). `delivery_type`이 비어있으면 소속 이벤트의 배송방식을 그대로 따름. `visible=false`면 고객 화면에서 이 리스팅만 숨김(카탈로그 상품 자체나 다른 회차 리스팅엔 영향 없음) — RLS도 `visible or is_admin()`으로 비노출 리스팅을 비관리자에게 숨김. `closed=true`는 `visible`과 반대로 화면엔 계속 보이되 주문만 막는 게이트(예약상품 발주마감 등, 재고와 무관) — 이벤트 전체의 `events.status`와는 별개로 리스팅 하나만 마감시킬 때 씀, `create_order` RPC가 여기 걸리면 트랜잭션 전체를 거부함. `order_deadline_at`(nullable)은 `closed`와 같은 효과를 내는 **예약된 자동 마감 시각** — 이 값이 지나면 `closed`를 직접 켜지 않아도 `isListingOrderable()`이 마감으로 판정하고, 값을 지우거나 미래로 늦추면 자동으로 다시 열림(관리자가 매번 수동으로 마감/해제 버튼을 누를 필요 없이 예약만 해두는 용도, `closed`와 서로 독립적으로 공존). `sort_order`는 이 이벤트 안에서의 노출 순서(관리자 ▲▼, Epic 1 Phase 4) — 이벤트마다 독립적이고, 마감시키면 자동으로 맨 뒤로 재정렬됨. 화면(`Product` 타입)에는 `products`와 조인한 평평한 모양으로 합쳐져서 전달됨(`catalogProductId`로 원본 카탈로그 id를 알 수 있음) |
