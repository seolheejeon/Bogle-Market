...
// 항상 type/isFlash와 동기화해야 하는 별도 컬럼을 두지 않기 위함. 관리자의 "종료"
// 버튼도 deadline_at을 지금으로 당기는 것 외엔 별도 취급 없이 이 정책을 그대로
// 따른다(문고리/택배는 종료를 눌러도 재고가 있으면 계속 주문된다).
export function getOrderPolicy(event: Pick<MarketEvent, "type" | "isFlash">): OrderPolicy {
  if (event.isFlash) return "STRICT_DEADLINE"; // 특가는 배송방식과 무관하게 마감이 곧 종료
...
export function isEventOrderable(event: Pick<MarketEvent, "type" | "isFlash" | "deadlineAt">): boolean {
